...
with open("autodock_rmsd_05.csv", "w") as f:
	wr = csv.writer(f,dialect= 'excel')
	title_row = ['Ligand_Name','Site_Number', 'With_Or_Without_Water', 'RMSD Value']
	append_list_as_row ('autodock_rmsd_05.csv', title_row)
f.close()

# Calculate rmsd and write into csv file
data = []
Ligands = ['Bicalein', 'Bicalin', 'Beta', 'DAR_DIR', 'Diammonium', 'Forsythoside_E', 'Lopinavir' 'VTRRT', 'Wogonin', 'Wogonoside']
...
for l in Ligands:
	for s in Sites:
		for w in Waters:
				...
				if len(all_files) == 2: #Change to 4 when i have ian's files
					...
					for af in all_files:
						...
						if len(traj.top.select('resname MOL')) != 0:
							...
							append_list_as_row('autodock_rmsd_05.csv', row_contents)
